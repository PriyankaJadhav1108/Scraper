"""
Salesforce (CRM) IR Quarterly Earnings Document Scraper
Extracts Press Releases, Presentations, Earnings Call Webcasts, and Transcripts
for the latest 2 calendar years via Salesforce's public IR feed API.

Output: crm_ir_docs/earnings_index.json
"""
...
BASE_URL = "https://investor.salesforce.com/overview/default.aspx"
QUARTERLY_URL = "https://investor.salesforce.com/financials/quarterly-results/default.aspx"
FINANCIAL_REPORT_API = (
    "https://investor.salesforce.com/feed/FinancialReport.svc/GetFinancialReportList?LanguageId=1"
)
DOWNLOAD_DIR = Path("crm_ir_docs")

# ─────────────────────────────────────────────
# SCRAPER
# ─────────────────────────────────────────────

async def scrape_salesforce_earnings(page, target_years: list[int]) -> list[dict]:
    print(f"\n{'─'*55}")
    print("  Fetching Salesforce financial report feed")
...
async def main(download_files: bool = False) -> list[dict]:
    ...
    async with async_playwright() as pw:
        ...
        print("\n  🌐 Loading Salesforce IR landing page...")
        await page.goto(BASE_URL, wait_until="domcontentloaded", timeout=60_000)
        await page.wait_for_timeout(2000)

        output_data = await scrape_salesforce_earnings(page, target_years)
        await browser.close()

    flat_output = finalize_company_output(output_data, "Salesforce")
